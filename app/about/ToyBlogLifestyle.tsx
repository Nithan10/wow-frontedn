...
export default function ToyBlogLifestyle({ isPreview = false, previewData = null }: { isPreview?: boolean, previewData?: any }) {
  ...
  if (isLoading || !data) {
    return (
      <div className={`${theme === 'dark' ? 'dark' : ''}`}>
        <div className="min-h-screen flex justify-center items-center bg-[#FAF8F5] dark:bg-[#0A0A0A]">
  ...
  return (
    <div className={`${theme === 'dark' ? 'dark' : ''}`}>
      <div className="relative min-h-screen bg-[#FAF8F5] dark:bg-[#0A0A0A] transition-colors duration-300">
        ...
        <div ref={containerRef} className={`${isPreview ? 'py-12' : 'pt-[70px]'} text-gray-900 dark:text-gray-100 selection:bg-[#D4AF37]/30`}>
              ...
              <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="grid lg:grid-cols-12 gap-16 items-start">
                <div className="lg:col-span-7">
                  <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="flex items-center gap-4 mb-10">
                    ...
                    <div className="h-px w-16 bg-gradient-to-r from-gray-300 dark:from-gray-700 to-transparent" />
                  ...
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
                    {featuredArticle.stats && Object.entries(featuredArticle.stats).map(([key, value]) => (
                      <div key={key} className="text-center p-4 bg-white/50 dark:bg-black/30 rounded-2xl border border-gray-100 dark:border-white/5 transition-colors">
          ...
          {/* 3. Heritage Timeline */}
          <section className="py-24 bg-gradient-to-b from-white to-gray-50 dark:from-[#0F0F0F] dark:to-[#1A1A1A] transition-colors">
                  ...
                  {timeline.map((item: any, index: number) => (
                    ...
                    >
                      <div className={`w-5/12 ${index % 2 === 0 ? 'text-right pr-12' : 'text-left pl-12'}`}>
                        <div className={`inline-block p-6 rounded-2xl bg-white dark:bg-[#111] border border-gray-100 dark:border-white/5 shadow-lg hover:shadow-xl transition-all ${
                      ...
                      <div className="absolute left-1/2 transform -translate-x-1/2 w-6 h-6 rounded-full bg-gradient-to-r from-[#D4AF37] to-amber-400 border-4 border-white dark:border-[#0F0F0F] shadow-lg transition-colors" />
                    </motion.div>
                  ))}
                </div>
              </div>
          ...
          <section className="py-24 relative overflow-hidden">
            <div className="absolute inset-0 opacity-20 dark:opacity-100 transition-opacity" style={{ backgroundImage: svgPattern }} />
          ...
          {/* 5. The Global Feed */}
          <section className="py-24 bg-gradient-to-b from-gray-50 to-white dark:from-[#0A0A0A] dark:to-[#111] transition-colors">
            <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="max-w-7xl mx-auto px-6 relative z-10">
              ...
              <div className="grid md:grid-cols-3 gap-8">
                {testimonials.map((testimonial: any, i: number) => (
                  <motion.div key={testimonial.id || i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="bg-white dark:bg-[#111] p-8 rounded-3xl border border-gray-100 dark:border-white/5 shadow-lg flex flex-col h-full transition-colors">
