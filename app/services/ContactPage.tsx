...
export default function ContactPage({ isOpen, onClose, isDarkMode, isPreview = false, previewData }: ContactPageProps) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.error('Error fetching contact info (Is your backend running?):', error);
      }
    };
    if (isOpen) fetchData();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setNotification(null); // Reset any existing notifications
    ...
    try {
      ...
      if (result.success) {
        ...
        // Delay closing the modal so the user can read the success message
        if (!isPreview) {
          setTimeout(() => {
            onClose();
            setNotification(null);
    ...
    <AnimatePresence>
      {notification && (
        <motion.div
          ...
          className={`fixed top-8 left-1/2 z-[10005] min-w-[320px] max-w-[90vw] flex items-center gap-3 px-5 py-4 rounded-xl shadow-2xl backdrop-blur-md border ${
        ...
        >
          {notification.type === 'success' ? (
            <CheckCircle size={22} className={isDarkMode ? 'text-green-400' : 'text-green-600'} />
          ) : (
            <AlertCircle size={22} className={isDarkMode ? 'text-red-400' : 'text-red-600'} />
          )}
  ...
  const ModalContent = (
    <>
      ...
      >
        ...
        <div className="md:w-3/5 p-6 md:p-8 overflow-y-auto bg-inherit no-scrollbar relative">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button 
              ...
              className="w-full py-4 bg-gradient-to-r from-yellow-400 to-amber-500 text-black font-black rounded-xl hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 text-sm md:text-base shadow-lg shadow-yellow-500/30 disabled:opacity-70 disabled:hover:scale-100"
            >
