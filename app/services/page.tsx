        ...
        <div className="max-w-7xl mx-auto mb-12">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-12">
            <div className="space-y-4 w-full lg:w-auto">
              ...
              <p className={`text-sm sm:text-lg max-w-2xl font-medium ${isDarkMode ? 'text-gray-400' : 'text-slate-600'}`}>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full lg:w-auto mt-4 lg:mt-0">
              ...
              <div className={`flex w-full sm:w-auto p-1.5 backdrop-blur-md rounded-xl shadow-2xl border ${
                ...
                <button onClick={() => { setViewMode('retail'); setSelectedProduct(0); }} className={`flex-1 sm:flex-none justify-center relative px-2 py-2.5 sm:px-8 sm:py-3 rounded-lg text-xs sm:text-sm font-black tracking-widest uppercase transition-all duration-300 flex items-center gap-2 sm:gap-3 ${
                  viewMode === 'retail' 
                ...
                <button onClick={() => { setViewMode('wholesale'); setSelectedProduct(0); }} className={`flex-1 sm:flex-none justify-center relative px-2 py-2.5 sm:px-8 sm:py-3 rounded-lg text-xs sm:text-sm font-black tracking-widest uppercase transition-all duration-300 flex items-center gap-2 sm:gap-3 ${
                  viewMode === 'wholesale' 
                    ? 'bg-gradient-to-r from-yellow-600 to-yellow-400 text-black shadow-[0_0_15px_rgba(234,179,8,0.3)]' 
                    : isDarkMode ? 'text-yellow-500/70 hover:text-yellow-400 hover:bg-white/5' : 'text-yellow-700/70 hover:text-yellow-600 hover:bg-black/5'
                }`}>
                ...
                <div className="overflow-x-auto w-full">
                  <table className="w-full min-w-[500px] sm:min-w-[600px]">
                    <thead className={`border-b ${isDarkMode ? 'bg-neutral-900/40 border-yellow-500/20' : 'bg-yellow-50/50 border-yellow-200'}`}>
                      <tr>
                        <th className={`text-left p-3 sm:p-4 text-[10px] sm:text-xs font-black uppercase tracking-widest ${isDarkMode ? 'text-yellow-600/80' : 'text-yellow-700'}`}>Product</th>
                        <th className={`text-left p-3 sm:p-4 text-[10px] sm:text-xs font-black uppercase tracking-widest ${isDarkMode ? 'text-yellow-600/80' : 'text-yellow-700'}`}>Category</th>
                        <th className={`text-left p-3 sm:p-4 text-[10px] sm:text-xs font-black uppercase tracking-widest ${isDarkMode ? 'text-yellow-600/80' : 'text-yellow-700'}`}>Price</th>
                        {viewMode === 'retail' ? (
                          <>
                            <th className={`text-left p-3 sm:p-4 text-[10px] sm:text-xs font-black uppercase tracking-widest ${isDarkMode ? 'text-yellow-600/80' : 'text-yellow-700'}`}>Discount</th>
                            <th className={`text-left p-3 sm:p-4 text-[10px] sm:text-xs font-black uppercase tracking-widest ${isDarkMode ? 'text-yellow-600/80' : 'text-yellow-700'}`}>Status</th>
                          </>
                        ) : (
                          <>
                            <th className={`text-left p-3 sm:p-4 text-[10px] sm:text-xs font-black uppercase tracking-widest ${isDarkMode ? 'text-yellow-600/80' : 'text-yellow-700'}`}>MOQ</th>
                            <th className={`text-left p-3 sm:p-4 text-[10px] sm:text-xs font-black uppercase tracking-widest ${isDarkMode ? 'text-yellow-600/80' : 'text-yellow-700'}`}>Margin</th>
                          </>
                        )}
                      </tr>
                    </thead>
                        ...
                        >
                          <td className="p-3 sm:p-4">
                            <div className="flex items-center gap-2 sm:gap-4">
                              <div className={`w-8 h-8 sm:w-12 sm:h-12 flex-shrink-0 rounded-xl flex items-center justify-center text-lg sm:text-2xl border shadow-lg group-hover:scale-110 transition-transform ${
                                isDarkMode ? 'bg-neutral-900 border-yellow-500/20' : 'bg-yellow-50 border-yellow-200'
                              }`}>
                                {product.icon}
                              </div>
                              <div>
                                <div className={`font-bold text-sm sm:text-base tracking-wide ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{product.name}</div>
                                <div className="flex items-center gap-2 mt-1">
                                  <span className={`text-[9px] sm:text-[10px] px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-md font-bold border ${
                          ...
                          <td className="p-3 sm:p-4">
                            <div className="flex flex-col">
                              <span className={`font-black text-base sm:text-xl whitespace-nowrap ${isDarkMode ? 'text-yellow-400' : 'text-yellow-600'}`}>{product.price}</span>
                          ...
                          {viewMode === 'retail' ? (
                            <>
                              <td className="p-3 sm:p-4"><span className="text-sm sm:text-base font-black text-green-500">{product.discount}</span></td>
                              <td className="p-3 sm:p-4"><span className={`text-[9px] sm:text-xs font-bold tracking-widest uppercase whitespace-nowrap ${product.stock === 'In Stock' ? (isDarkMode ? 'text-yellow-400' : 'text-yellow-600') : 'text-red-500'}`}>{product.stock}</span></td>
                            </>
                          ) : (
                            <>
                              <td className="p-3 sm:p-4"><span className={`text-[10px] sm:text-sm font-bold px-2 py-1 rounded-lg whitespace-nowrap ${isDarkMode ? 'text-white bg-neutral-800' : 'text-slate-800 bg-slate-100'}`}>{product.moq}</span></td>
                              <td className="p-3 sm:p-4"><span className="text-sm font-bold text-green-500">{product.margin}</span></td>
                            </>
                          )}
                        </tr>
                      ))}
                    ...
                    <div className="relative z-10 flex flex-col h-full">
                      ...
                      <div className="mb-6 sm:mb-10 text-center">
                        <div className="relative inline-block">
                          ...
                          <div className="absolute -top-3 -right-4 sm:-top-4 sm:-right-6 animate-pulse">
                        ...
                        <div className={`text-base sm:text-xl font-black mb-2 sm:mb-3 tracking-widest uppercase mt-3 sm:mt-4 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{currentOffer.title}</div>
                      ...
                      {currentProducts[selectedProduct] && (
                        <div className={`mt-2 p-3 sm:p-5 rounded-2xl border mb-6 sm:mb-8 ${
                          ...
                          <div className={`flex items-center justify-between gap-3 sm:gap-4 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                            <div className="flex-1 min-w-0">
                              <div className="font-bold text-xs sm:text-base truncate">{currentProducts[selectedProduct].name}</div>
                            </div>
                            <div className="text-right flex-shrink-0">
                              <div className={`text-base sm:text-xl font-black ${isDarkMode ? 'text-yellow-400' : 'text-yellow-600'}`}>{currentProducts[selectedProduct].price}</div>
                            </div>
                          </div>
                        </div>
                      )}
                      ...
                      {/* CTA Button */}
                      <button className="mt-auto w-full py-3 sm:py-4 bg-gradient-to-r from-yellow-600 via-yellow-400 to-yellow-600 bg-[length:200%_auto] hover:bg-[position:right_center] text-black font-black tracking-widest uppercase rounded-xl transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 shadow-[0_0_20px_rgba(234,179,8,0.3)] hover:shadow-[0_0_30px_rgba(234,179,8,0.5)] hover:scale-[1.02] active:scale-[0.98] text-xs sm:text-base">
